  ...
  try {
    ...
    const invoice = await prisma.$transaction(async (tx: any) => {
      ...
      for (const item of items) {
        if (!item.MaSP) throw new Error('Dữ liệu giỏ hàng bị hỏng (Thiếu mã sản phẩm). Vui lòng làm trống giỏ hàng.');
        const product = await tx.sanPham.findUnique({
          where: { MaSP: item.MaSP },
          include: { LoHangs: { where: { SoLuongTon: { gt: 0 } }, orderBy: { HanSuDung: 'asc' } } }
        });

        if (!product) throw new Error(`Product ${item.MaSP} not found`);
        ...
        if (remainingToDeduct > 0) throw new Error(`Not enough stock for ${product.TenSP}`);
