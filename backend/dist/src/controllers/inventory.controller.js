...
exports.importExcel = exports.getInventoryAlerts = exports.getInventory = exports.receiveGoods = void 0;
...
const receiveGoods = async (req, res) => {
    try {
        ...
        const newBatch = await index_1.prisma.loHang.create({
            data: {
                MaSP: Number(MaSP),
        ...
        const currentProduct = await index_1.prisma.sanPham.findUnique({
            where: { MaSP: Number(MaSP) }
        });
        await index_1.prisma.sanPham.update({
            where: { MaSP: Number(MaSP) },
...
const getInventory = async (req, res) => {
    try {
        ...
        const inventory = await index_1.prisma.loHang.findMany({
            where: {
                ...batchWhereClause,
                SanPham: {
                    ...productWhereClause
...
const getInventoryAlerts = async (req, res) => {
    try {
        ...
        const [lowStockCount, expiringCount] = await Promise.all([
            index_1.prisma.loHang.count({ where: { SoLuongTon: { lt: 10, gt: 0 } } }),
            index_1.prisma.loHang.count({ where: { HanSuDung: { lte: thirtyDaysFromNow }, SoLuongTon: { gt: 0 } } })
        ]);
